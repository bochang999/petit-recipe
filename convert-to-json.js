...
function validateRecipeData(data) {
  ...
  for (let i = 0; i < data.recipes.length; i++) {
    ...
    if (
      ...
      return {
        valid: false,
        error: `Recipe ${recipe.name}: Missing ingredients`,
      };
    }
    ...
    for (let ingredient of recipe.ingredients) {
      if (
        ...
        return {
          valid: false,
          error: `Recipe ${recipe.name}: Invalid ingredient format`,
        };
      }
    }
  }
...
console.log(
  "Recipe conversion script loaded. Use convertLegacyToJSON() to convert data.",
);
