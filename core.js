...
const app = {
  ...
  populateEditForm() {
    ...
    if (instructionsInput) {
      // Use 'steps' field from recipe data (actual field name in recipes.json)
      const instructions = this.selectedRecipe.steps || this.selectedRecipe.instructions || [];
   ...
   */
  async addRecipe(recipeData) {
    console.log("➕ Adding new recipe:", recipeData.name);
    ...
    try {
      ...
      // Save to file using recipeDataManager
      if (window.recipeDataManager) {
        await window.recipeDataManager.saveRecipes(this.recipes);
        console.log("✅ Recipe saved to file");
      } else {
  ...
  async updateRecipe(recipeData) {
    console.log("✏️ Updating recipe:", recipeData.id, recipeData.name);

    try {
      // Find and update recipe in local array
  ...
  async handleRecipeFormSubmit(event) {
    ...
    try {
      ...
      // Convert instructions from text to array
      ...
      const recipeData = {
        ...
        steps: instructionsArray, // Use 'steps' to match existing data structure
        servings: 4, // Default value
        cookTime: "30分" // Default value
      };

      if (this.editMode && this.editingRecipeId) {
        // Edit mode - update existing recipe
        recipeData.id = this.editingRecipeId;
        await this.updateRecipe(recipeData);
        console.log("✅ Recipe updated successfully");
      } else {
        // Add mode - create new recipe
        await this.addRecipe(recipeData);
      ...
      console.error("❌ Recipe form submission failed:", error);
      alert(`❌ レシピの保存に失敗しました: ${error.message}`);
    }
  },
