...
class LocalRecipeManager {
    ...
    async initialize() {
        try {
            ...
            if (hasLocal) {
                ...
                console.log(`✅ Loaded ${this.recipes.length} recipes from local storage`);
            } else {
                // First time - migrate from embedded data
                await this.migrateFromEmbedded();
                console.log(`✅ Migrated ${this.recipes.length} recipes to local storage`);
            }
    ...
    async addRecipe(recipe) {
        try {
            ...
            // Generate new ID
            const maxId = this.recipes.reduce((max, r) => Math.max(max, parseInt(r.id) || 0), 0);
