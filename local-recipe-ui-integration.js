    ...
    try {
        ...
        if (recipe.ingredients.length > 0 && typeof recipe.ingredients[0] === 'string') {
            ...
            recipe.ingredients = recipe.ingredients.map(ing => {
                const match = ing.match(/^(.+?)\\s+(\\d+(?:\\.\\d+)?)\\s*([^\\d\\s]+)$/);
                if (match) {
                    return {
                        name: match[1].trim(),
        ...
        console.log(`✅ Recipe added locally: ${addedRecipe.name} (ID: ${addedRecipe.id})`);
        return addedRecipe;
    } catch (error) {
...
window.updateRecipeInLocal = async function(recipeId, recipeData) {
    try {
        ...
        const updatedRecipe = await window.localRecipeManager.updateRecipe(recipeId, recipe);
...
window.deleteRecipeFromLocal = async function(recipeId) {
    try {
        ...
        const deletedRecipe = await window.localRecipeManager.deleteRecipe(recipeId);
