...
- **署名の不一致** (最頻発): 
  - 原因: 異なるバージョンで異なる署名キーが使用された
  - 解決: 古いアプリを完全アンインストール → 端末再起動 → 新APKインストール
  - 予防: GitHub Actions自動署名により統一キーストアで解決済み
- **容量不足**: Android設定 > ストレージでアプリ用容量確保 (最低100MB推奨)
- **Playプロテクト干渉**: Google Play プロテクト一時無効化
- **キャッシュ問題**: 端末設定 > アプリ > パッケージインストーラ > ストレージ > キャッシュを削除

### 🔄 確実な更新手順（推奨）
1. **事前バックアップ**: 重要なアプリデータを確認
2. **完全アンインストール**: 設定 > アプリ > Petit Recipe > アンインストール
3. **端末再起動**: アイコンキャッシュとインストール履歴をクリア
4. **最新APKインストール**: ダウンロード後にインストール実行
5. **動作確認**: アプリ起動と基本機能テスト
